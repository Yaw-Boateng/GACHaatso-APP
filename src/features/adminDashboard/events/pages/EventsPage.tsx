...
import api, { IMAGE_BASE_URL } from "../../../../services/apiClient";
...
// Helper: Safely convert string to valid Date
...
// Helper: Resolve relative vs absolute image URLs
const resolveImageUrl = (url?: string): string => {
  if (!url) return "/photos/1448709/pexels-photo-1448709.jpeg";
  const trimmed = url.trim();
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    return trimmed;
  }
  return `${IMAGE_BASE_URL}${trimmed.startsWith("/") ? "" : "/"}${trimmed}`;
};
...
const EventsPage = () => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (result && typeof result === "object") {
          const list = result.data?.content || result.content || (Array.isArray(result) ? result : []);
          const pages = result.data?.totalPages || result.totalPages || 1;
        ...
        const serverErrorMessage =
          typeof err === "object" && err !== null && "response" in err
            ...
            ? err.message
            : undefined;
        setError(serverErrorMessage || "An unexpected networking issue occurred.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredEvents = events.filter((event) => {
    ...
    const matchesCategory =
      selectedCategory === "All" || (event.category || "All") === selectedCategory;
    ...
    <div className="min-h-screen pt-16 bg-neutral-50 dark:bg-[#090a0f] text-neutral-900 dark:text-neutral-100 antialiased transition-colors duration-300">
      ...
      <section className="relative py-20 bg-primary-900 dark:bg-neutral-900 text-white overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-neutral-800/20 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2" />
        </div>
        ...
        <div className="container mx-auto px-4 relative z-10 text-center md:text-left">
          <div className="max-w-3xl">
            ...
            <p className="text-lg md:text-xl text-white dark:text-neutral-400 font-sans leading-relaxed">
              Discover opportunities to connect, learn, and collaborate with our local community members.
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-4">
            ...
            <p className="text-xs text-neutral-400">Loading catalog matrix content stream...</p>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-24 text-center border border-neutral-200 dark:border-neutral-800 rounded-2xl bg-white dark:bg-[#0d0e16] p-6">
            <h3 className="text-sm font-semibold text-red-500 mb-1">Failed to connect to event directory</h3>
          ...
          <>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {sortedEvents.map((event) => {
                ...
                const targetSrc = resolveImageUrl(event.imageUrl);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 text-center">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-xs text-neutral-400 dark:text-neutral-500 mb-6">
              Get clean digests containing coming scheduled events sent direct to your mailbox weekly.
            </p>
