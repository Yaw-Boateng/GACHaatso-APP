...
const NAV_ITEMS: NavItem[] = [
  {
    ...
    roles: ["ADMIN", "LEADER", "PASTOR", "PASTORS"],
  },
  {
    label: "Events Manager",
    path: "/dashboard/eventsdb",
    icon: Sparkles,
    roles: ["ADMIN", "PASTOR", "PASTORS"],
  },
  {
    label: "Group Management",
    path: "/dashboard/group",
    icon: Users,
    roles: ["LEADER"], // Restricted strictly to LEADER
  },
  {
    label: "Attendance",
    ...
    <>
      ...
      >
        ...
        <div className="pt-4 border-t border-neutral-200/60 dark:border-neutral-800/60 space-y-1">
          <button
            onClick={toggleTheme}
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900/50 hover:text-neutral-900 dark:hover:text-neutral-100 text-sm font-medium transition-all"
          >
            {theme === "dark" ? (
              <>
      ...
      {/* PERMANENT DESKTOP SIDEBAR PANEL */}
      <aside className="w-64 hidden lg:flex flex-col h-screen sticky top-0 border-r border-neutral-200/60 dark:border-neutral-800/60 bg-white dark:bg-[#0d0e16] transition-colors duration-300 shrink-0">
        ...
        <div className="p-3 border-t border-neutral-200/60 dark:border-neutral-800/60 space-y-0.5">
          <button
            onClick={toggleTheme}
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900/60 text-sm font-medium transition-all"
          >
            {theme === "dark" ? (
              <>
                <Sun size={18} className="text-amber-500" />
          ...
          <button
            onClick={handleSignOut}
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-neutral-400 dark:text-neutral-500 hover:bg-red-50 dark:hover:bg-red-950/10 hover:text-red-500 dark:hover:text-red-400 text-sm font-medium transition-all"
          >
            <LogOut size={18} />
            <span>Sign Out</span>
          </button>
